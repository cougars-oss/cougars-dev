...
void WaypointFollowerNode::waypointCallback(const geometry_msgs::msg::PoseArray::SharedPtr msg)
{
  ...
  total_waypoints_.store(waypoints_.size());
  current_waypoint_index_.store(0);
  state_.store(MissionState::ACTIVE);
  previous_distance_ = -1.0;
  current_dist_to_target_.store(-1.0);
...
{
  last_odom_time_seconds_.store(this->get_clock()->now().seconds());

  if (state_.load() == MissionState::IDLE) {
    return;
  }
...
{
  const auto & target = waypoints_[current_waypoint_index_.load()];
  double distance = calculateDistance(current_x, current_y, target.position.x, target.position.y);
  current_dist_to_target_.store(distance);
...
{
  if (state_.load() == MissionState::IDLE || last_odom_time_seconds_.load() == 0.0) {
    return;
  }

  double now = this->get_clock()->now().seconds();
  if ((now - last_odom_time_seconds_.load()) > params_.odom_timeout_sec) {
...
{
  state_.store(MissionState::IDLE);
  waypoints_.clear();
  total_waypoints_.store(0);
  current_dist_to_target_.store(0.0);
...
void WaypointFollowerNode::checkMissionStatus(diagnostic_updater::DiagnosticStatusWrapper & stat)
{
  if (state_.load() == MissionState::IDLE) {
    ...
    stat.summary(
      ...
      "Mission received. Navigating to waypoint " +
      std::to_string(current_waypoint_index_.load() + 1) +
...
{
  if (last_odom_time_seconds_.load() == 0.0) {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN, "No Odometry received.");
    return;
  }

  double time_since = this->get_clock()->now().seconds() - last_odom_time_seconds_.load();
